...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            place.ant = self
        elif isinstance(place.ant, ContainerAnt) and place.ant.can_contain(self) and not isinstance(self, ContainerAnt):
    ...
    def remove_from(self, place):
        if place.ant is self:
            ...
            if not isinstance(self, ContainerAnt):
                place.ant.remove_ant(self)
            else:
                place.ant.remove_ant(self)
                self.contained_ant.add_to(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while distance < self.min_range:
            if not p.entrance or p is beehive:
                return None
            p = p.entrance
            distance += 1

        while not p.bees and p.entrance:
            p = p.entrance
            distance += 1
        if p.bees and p is not beehive and distance <= self.max_range:
            return rANTdom_else_none(p.bees) # REPLACE THIS LINE
        else:
            return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # targets = self.place.bees[:]
        # for target in targets:
        #     for t in self.place.bees:
        #         if t == target:
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.check > 1:
            ...
            QueenAnt.throw_at(self, self.nearest_bee(gamestate.beehive))
            location = self.place.exit
            while location:
                if location.ant and location.ant.buff == 1:
                    location.ant.buff = 2
                    location.ant.damage = location.ant.buff * location.ant.damage
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.is_scare and self.scare < 2:
        ...
        elif self.armor > 0 and destination is not None and not isinstance(destination, Hive):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        location = self.place
        distance = 0
        while location is not beehive:
            for bb in location.bees:
                insects[bb] = distance
            if location.ant:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        harm_value = self.damage - 0.05 * self.insects_shot - 0.2 * distance
        if harm_value >= 0:
            return harm_value
        else:
            return 0
...
# print('bee number', len(place.bees))               # How many bees are there?
